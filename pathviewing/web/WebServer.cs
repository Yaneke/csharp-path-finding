...
using PathViewing.Web.DataObjects;

namespace PathViewing.Web {
    class HttpServer {
        private HttpListener listener;
        private GridGraph map;
        private CBS cbs;
        private IEnumerator<CBSNode> cbsEnumerator;
        private PathRequestDO prevRequest;

        private string HTML_DIR = "web/html/";
        private string DATA_DIR = "../common/data/";
        ...
        private void UpdateMapSettings(HttpListenerRequest req, HttpListenerResponse resp) {
            ...
            MapSettingsDO settings = JsonSerializer.Deserialize<MapSettingsDO>(data);
            string fileName = DATA_DIR + settings.map;
        ...
        private void SetMapSettings(HttpListenerRequest req, HttpListenerResponse resp) {
            string data = this.ReadPostData(req);
            try {
                MapSettingsDO constraintUpdate = JsonSerializer.Deserialize<MapSettingsDO>(data);
        ...
        private void UpdateConstraints(HttpListenerRequest req, HttpListenerResponse resp) {
            string data = this.ReadPostData(req);
            try {
                ConstraintUpdateDO constraintUpdate = JsonSerializer.Deserialize<ConstraintUpdateDO>(data);
                this.cbs = new CBS();
        ...
        private void GetPath(HttpListenerRequest req, HttpListenerResponse resp) {
            try {
                string data = this.ReadPostData(req);
                PathRequestDO pathRequests = JsonSerializer.Deserialize<PathRequestDO>(data);
                List<Vertex> sources = pathRequests.GetSources(this.map);
                List<Vertex> destinations = pathRequests.GetDestinations(this.map);
                Stopwatch sw = new Stopwatch();
                sw.Start();
                Solution sol = this.cbs.ShortestPath(this.map, sources, destinations);
                sw.Stop();
                var res = new PathAnswerDO(sol, sw.ElapsedMilliseconds);
        ...
        private void GetPathStep(HttpListenerRequest req, HttpListenerResponse resp) {
            try {
                ...
                if (!this.cbsEnumerator.MoveNext()) {
                    ...
                    sw.Stop();
                    var res = new CBSNodeDO(this.cbs.checkers, this.cbsEnumerator.Current, sw.ElapsedMilliseconds);
                    byte[] responseData = JsonSerializer.SerializeToUtf8Bytes(res);
                    resp.ContentType = "text/json";
                    resp.ContentLength64 = responseData.LongLength;
                    resp.ContentEncoding = Encoding.UTF8;
